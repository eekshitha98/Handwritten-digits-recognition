...
def initialize_network(n_inputs, n_hidden, n_outputs):
	network = []
	hidden_layer = [{'weights':[uniform(-0.5,0.5) for i in range(n_inputs + 1)]} for i in range(n_hidden)]
	network.append(hidden_layer)
	output_layer = [{'weights':[uniform(-0.5,0.5) for i in range(n_hidden + 1)]} for i in range(n_outputs)]
	network.append(output_layer)
	return network


def assign_values(network,arr):
	index = 0;
	for layer in network:
		for neuron in layer:
			for i in range(len(neuron['weights'])):
				neuron['weights'][i] = arr[index]
				index += 1
...
def train_network(network, train, l_rate, n_epoch, n_outputs,output_labels,matrix):
	for epoch in range(n_epoch):
		...
		for row in train:
			...
			expected[output_labels[index]] = 1
			print('output of neural network is %d \n', output_labels[index] )
			print(outputs)
...
#arr = np.loadtxt('tempo')
#print(network)
#assign_values(network,arr)
train_network(network, data, 0.5, 500, n_outputs,labels,matrix)

'''

with gzip.open('t10k-images-idx3-ubyte.gz', 'rb') as f:
            testset = np.frombuffer(f.read(), np.uint8, offset=16)

testset = testset.reshape(10000,28*28)

testset = testset/np.float32(256)
